...
arbVars inputs mids =
  ...
  where
    varInps _inputs = [Var . InputWire ("", Nothing) Public <$> elements _inputs]
    varMids [] = []
...
prop_complexMultiplication (a, b, c, d) =
  let circuit =
        ArithCircuit
          [ Mul (Var (InputWire ("", Nothing) Public 1)) (Var (InputWire ("", Nothing) Public 2)) (OutputWire 3),
            Mul (Var (InputWire ("", Nothing) Public 4)) (Var (InputWire ("", Nothing) Public 5)) (OutputWire 6),
...
prop_division (a, b) =
  let circuit =
        ArithCircuit
          [ Mul (Var (InputWire ("", Nothing) Public 1)) (Var (InputWire ("", Nothing) Public 5)) (IntermediateWire 3),
            Mul (ConstGate 1) (ConstGate 1) (IntermediateWire 4),
            Mul (Var (InputWire ("", Nothing) Public 2)) (Var (IntermediateWire 5)) (OutputWire 4)
          ]
      inputs = IntMap.fromList [(1, a), (2, b)]
      vars = collectCircuitVars circuit
      solution = solve vars circuit inputs
...
prop_bitSummingForward a =
  let circuit =
        ArithCircuit
          [ Split (InputWire ("", Nothing) Public 1) (OutputWire <$> [2 .. nBits + 1])
          ]
      -- forward
      vars = collectCircuitVars circuit
